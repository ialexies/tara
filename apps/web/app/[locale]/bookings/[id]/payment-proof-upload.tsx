  ...
  async function handleFile(file: File) {
    ...
    try {
      const { uploadUrl, publicUrl } = await publicFetch<{ uploadUrl: string; publicUrl: string }>(
        `/bookings/${bookingId}/payment-proof-upload-url`,
        { method: 'POST', body: JSON.stringify({ contentType: file.type, guestEmail }) },
      );
      ...
      await publicFetch(`/bookings/${bookingId}/payment-proof`, {
        method: 'POST',
        body: JSON.stringify({ proofUrl: publicUrl, guestEmail }),
      });
      setDone(true);
    } catch (e: unknown) {
    ...
    return (
      <div className="rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-4 dark:border-emerald-900 dark:bg-emerald-950">
        ...
        <p className="mt-0.5 text-xs text-emerald-700 dark:text-emerald-300">
          The property will confirm your booking once they verify your payment.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900">
      ...
      <p className="mb-3 text-xs text-zinc-500">
        Take a screenshot of your GCash, Maya, or bank transfer confirmation and upload it here so
        the property can verify your payment faster.
      </p>
